...
public class RubySerializerTest {
	...
	public void writeWithMethodCallback() {
		...
		// setup
		RubySerializer parser = new RubySerializer();
		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
	...
	public void writeWithMethodCallbackNonAlphaNum() {
	 ...
	 @Test
	    public void CouldWriteNullValue(){
				...
				UniversalSerializer.getInstance().write(byteArrayOutputStream,null, OutputFormat.RUBY);
				Assert.assertEquals("nil",byteArrayOutputStream.toString());
	    		String actual = UniversalSerializer.getInstance().writeToString(null, OutputFormat.RUBY);
	    		Assert.assertEquals("nil",actual);
	    }
