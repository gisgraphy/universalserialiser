...
//derived from org.codehaus.jackson.Utf8Generator
public class PythonGenerator extends JsonGeneratorBase {
    ...
    protected final void _writePPFieldName(SerializedString name, boolean commaBefore) throws IOException, JsonGenerationException {
	if (commaBefore) {
	    _cfgPrettyPrinter.writeObjectEntrySeparator(this);
	} else {
	    _cfgPrettyPrinter.beforeObjectEntries(this);
    ...
    private final void _writeString(String text) throws IOException, JsonGenerationException {
    ...
    private final void _writeSegmentedString(String text) throws IOException, JsonGenerationException {
	int offset = 0;
	int len = text.length();
	final char[] buf = _charBuffer;
	do {
	...
	main_loop: while (offset < len) {
		...
		ptr = _writeEscapedControlChar(escape, ptr);
		 ...
		 //NEED MIGRATION
		//REMOVE outputBuffer[ptr++] = (byte) (0xc0 | (ch >> 6));
		//REMOVE outputBuffer[ptr++] = (byte) (0x80 | (ch & 0x3f));
		//ADD
		outputBuffer[ptr++] = '\\';
		outputBuffer[ptr++] = 'u';
		outputBuffer[ptr++] = (byte) hexdigits[(ch >>> 12)];
		outputBuffer[ptr++] = (byte) hexdigits[(ch >>> 8) & 0xf];
		outputBuffer[ptr++] = (byte) hexdigits[(ch >>> 4) & 0xf];
		outputBuffer[ptr++] = (byte) hexdigits[(ch) & 0xf];
		//END ADD
    ...
    private void _writeSegmentedString(char[] cbuf, int offset, int len) throws IOException, JsonGenerationException {
	// Fast loop for chars not needing escaping
	len += offset; // becomes end marker, then
	int ptr = _outputTail;
	...
	main_loop: while (offset < len) {
	    final int[] escCodes = CharTypes.getOutputEscapes();
	    final int end = _outputEnd - 6; // let's always have room for 6 more
					    // bytes, simpler
	    final byte[] outputBuffer = _outputBuffer;
	...
	if (ch >= SURR1_FIRST && ch <= SURR2_LAST) { // yes, outside of BMP;
						     // add an escape
	    ...
	    bbuf[outputPtr++] = '\\';
	    bbuf[outputPtr++] = 'u';
	    bbuf[outputPtr++] = (byte) hexdigits[(ch >>> 12)];
	    bbuf[outputPtr++] = (byte) hexdigits[(ch >>> 8) & 0xf];
	    bbuf[outputPtr++] = (byte) hexdigits[(ch >>> 4) & 0xf];
	    bbuf[outputPtr++] = (byte) hexdigits[(ch) & 0xf];
	    //ADD
	}
	return outputPtr;
    }
    ...
    /**
     * @param escCode
     *            Character code for escape sequence (\C); or -1 to indicate a
     *            generic (\\uXXXX) sequence.
     */
